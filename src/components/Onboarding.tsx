...
type StepId = 'preflight' | 'theme' | 'provider' | 'memory' | 'oauth' | 'api-key' | 'security' | 'terminal-setup';
...
export function Onboarding({
  ...
  const onboardingPreset = getAlfredOnboardingPresetFromEnv();
  const onboardingModel = getAlfredOnboardingModelFromEnv();
  ...
  const memoryStep = <Box flexDirection="column" gap={1} paddingLeft={1}>
      <Text bold>Optional memory companion</Text>
      <Box flexDirection="column" width={70} gap={1}>
        <Text>
          Alfred can connect to a running Claude-Mem compatible worker for
          persistent memory across sessions.
        </Text>
        <Text dimColor>
          Default endpoint: {getAlfredMemoryStatus().endpoint}
        </Text>
        <Select options={[{
          label: 'Enable if companion is running',
          value: 'enable'
        }, {
          label: 'Skip for now',
          value: 'skip'
          ...
          void probeAlfredMemoryEndpoint(endpoint).then(healthy => {
            if (healthy) {
              setAlfredMemoryStatus({
                enabled: true,
                endpoint
        ...
        <Text dimColor>
          If no worker is running yet, Alfred continues normally and you can enable it later with `/alfred-memory`.
        </Text>
      </Box>
    </Box>;
  ...
  steps.push({
    id: 'provider',
    component: <ProviderManager mode="first-run" initialPreset={onboardingPreset} initialModel={onboardingModel} onDone={result => {
  ...
  if (oauthEnabled) {
    steps.push({
      id: 'oauth',
      component: <SkippableStep skip={skipOAuth || providerConfigured} onSkip={goToNextStep}>
