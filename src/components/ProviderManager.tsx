...
import type { AlfredProviderChoice } from '../utils/alfredOnboarding.js'
import {
...
type Props = {
  ...
  initialPreset?: AlfredProviderChoice
  initialModel?: string
...
function CodexOAuthSetup({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        Sign in with your ChatGPT account in the browser. Alfred will store
...
export function ProviderManager({
  ...
  const hasAppliedInitialPresetRef = React.useRef(false)
  ...
  React.useEffect(() => {
    if (hasAppliedInitialPresetRef.current || !initialPreset) {
      return
    }

    hasAppliedInitialPresetRef.current = true

    if (initialPreset === 'codex-oauth') {
      setScreen('codex-oauth')
      return
    }

    const defaults = getProviderPresetDefaults(initialPreset)
    const nextDraft = {
      name: defaults.name,
      baseUrl: defaults.baseUrl,
      model: initialModel?.trim() || defaults.model,
      apiKey: defaults.apiKey ?? '',
    }
  ...
  function buildCodexOAuthActivationMessage(options: {
    ...
    if (options.warnings.length > 0) {
      return `${options.prefix}. Alfred switched to it for this session with warnings: ${options.warnings.join('; ')}.`
    }

    return `${options.prefix}. Alfred switched to it for this session.`
  }
  ...
  function renderPresetSelection(): React.ReactNode {
    ...
    const options = [
      ...
      },
      {
        value: 'openrouter',
        label: 'OpenRouter',
        description: 'OpenRouter endpoint with easy models like openrouter/auto or openrouter/free',
      },
      {
        value: 'lmstudio',
        label: 'LM Studio',
      ...
      ...(mode === 'first-run'
        ? [
            {
              value: 'skip',
              label: 'Use built-in Claude login instead',
              description: 'Skip provider setup and continue to the Claude account login flow',
            },
          ]
        : []),
    ]
    ...
    return (
      <Box flexDirection="column" gap={1}>
        <Text color="remember" bold>
          {mode === 'first-run' ? 'Set up Alfred' : 'Choose provider preset'}
        </Text>
        <Text dimColor>
          Pick a provider, then confirm the base URL, model, and API key Alfred should use.
        </Text>
        <Select
          options={options}
          onChange={(value: string) => {
