...
import {
  ALFRED_AUTH_CHOICE_ENV,
  ALFRED_MODEL_ENV,
  ALFRED_ONBOARDING_FORCE_ENV,
...
async function main(): Promise<void> {
  ...
  if (args[0] === 'onboard') {
    process.env[ALFRED_ONBOARDING_FORCE_ENV] = '1'
    ...
    for (let index = 1; index < args.length; index += 1) {
      ...
      if (arg === '--auth-choice') {
        ...
        if (value) {
          process.env[ALFRED_AUTH_CHOICE_ENV] = value
          index += 1
        }
        continue
      }
      if (arg.startsWith('--auth-choice=')) {
        process.env[ALFRED_AUTH_CHOICE_ENV] = arg.slice('--auth-choice='.length)
        continue
      }
      if (arg === '--model') {
        const value = args[index + 1]
        if (value) {
          process.env[ALFRED_MODEL_ENV] = value
          index += 1
        }
        continue
      }
      if (arg.startsWith('--model=')) {
        process.env[ALFRED_MODEL_ENV] = arg.slice('--model='.length)
        continue
      }
      nextArgs.push(arg)
    }

    process.argv = [process.argv[0]!, process.argv[1]!, ...nextArgs]
    args = nextArgs
  }
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (OpenClaude / Alfred)`);
    return;
  }
