# Alfred on Android (Termux)

A complete guide to running Alfred on Android using Termux + proot Ubuntu.
...
Alfred requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
Both were tested and fail due to Alfred's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the Alfred repo.
