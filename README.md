...
```bash
npm install -g alfred-openclaude
```
...
Inside Alfred / OpenClaude:

- run `/provider` for guided provider setup and saved profiles
- run `/onboard` to re-run Alfred provider onboarding
- run `/onboard-github` for GitHub Models onboarding

### Alfred one-command onboarding

Start the guided Alfred flow directly:

```bash
openclaude onboard
```

Preselect a provider path:

```bash
openclaude onboard --auth-choice openai-codex
openclaude onboard --auth-choice openrouter --model openrouter/free
openclaude onboard --auth-choice openai
```

Supported `--auth-choice` values include `openai`, `openai-codex`, `chatgpt-oauth`, `openrouter`, `ollama`, `anthropic`, and `custom`.
...
| --- | --- | --- |
| OpenAI-compatible | `/provider`, `/onboard`, or env vars | Works with OpenAI, OpenRouter, DeepSeek, Groq, Mistral, LM Studio, and other compatible `/v1` servers |
| Gemini | `/provider` or env vars | Supports API key, access token, or local ADC workflow on current `main` |
| GitHub Models | `/onboard-github` | Interactive onboarding with saved credentials |
| Codex OAuth | `/provider` or `/onboard` | Opens ChatGPT sign-in in your browser and stores Codex credentials securely |
...
- **Alfred workflow bundle**: ECC-inspired bundled skills for planning, search-first, TDD, review, verification, and security
- **Optional memory companion**: Connect to a running Claude-Mem compatible worker with `/alfred-memory`
