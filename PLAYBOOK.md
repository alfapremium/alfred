# Alfred Local Agent Playbook

This playbook is a practical guide to run Alfred with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, Alfred starts directly.
...
Or let Alfred recommend the best local model for your goal:
...
```powershell
Get-Content .\.alfred-profile.json
```
...
- Keep `.alfred-profile.json` local (already gitignored).
